...
test("both surfaces wire Settings to the shared store and destructive confirmation", () => {
  ...
  for (const file of ["Panel.qml", "Expanded.qml"]) {
    ...
    for (const prop of ["updateDataPaths", "updateDataLoading", "cleanupOutcome", "cleanupEnabled"])
      assert.match(info, new RegExp(`store\\.${prop}`), `${file} ${prop}`)
    assert.match(info, /updateDataArchivePath: store\.updateDataPaths \? store\.updateDataPaths\.archive : ""/)
    assert.doesNotMatch(info, /updateDataCount/)
