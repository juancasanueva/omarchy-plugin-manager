...
test("SettingsInfo shares safe, content-sized Info and About cards", () => {
  ...
  for (const label of ["Plugins installed in", "Updates are in", "Updates Archives are in"])
    assert.ok(source.includes(`label: "${label}"`))
  assert.doesNotMatch(source, /Plugin Manager Updates/)
  const label = byId(all, "pathLabel")
  const path = byId(all, "pathValue")
  assert.equal(label.parent, path.parent)
  assert.match(label.body, /color: root\.secondaryForeground/)
  assert.match(path.body, /color: Color\.accent/)
  assert.match(path.body, /text: root\.displayPath\(entry\.path\)/)
  assert.match(path.body, /font\.bold: true/)
  for (const text of [label, path]) assert.match(text.body, /width: parent\.width/)
  assert.equal(all.filter(object => object.type === "PathEntry").length, 3)
  assert.doesNotMatch(source, /updateDataCount|updateDataCounter|Active entries only; archive excluded/)
...
for (const file of ["Panel.qml", "Expanded.qml"]) {
  ...
  test(`${file}: shared metadata follows Restart Shell in the scroll content`, () => {
    ...
    assert.match(info.body, /updateDataArchivePath: store\.updateDataPaths \? store\.updateDataPaths\.archive : ""/)
    assert.doesNotMatch(info.body, /updateDataCount/)
